...
struct PlannedRename {
    old_path: PathBuf,
    new_path: PathBuf,
    warn: bool, // true if season or episode equals "0"
}
...
fn check_warning(original: &str, re: &Regex) -> bool {
...
fn should_process_file(path: &Path, allowed_types: &[String]) -> bool {
...
fn main() {
    ...
    let re = Regex::new(&cli.current_pattern)
        .expect("Invalid regex pattern provided for current file names.");
    ...
    if planned.iter().any(|p| p.warn) {
        eprintln!("Warning: Some files have season or episode as 0. This might be unintended.");
        ...
        if input != "y" && input != "yes" {
            eprintln!("Aborting as per user request.");
            return;
        }
    }
    ...
    for plan in planned {
        println!("Renaming:\n  From: {:?}\n  To:   {:?}", plan.old_path, plan.new_path);
        if cli.dry_run {
            println!("Dry-run mode: no changes made.\n");
        } else {
            fs::rename(&plan.old_path, &plan.new_path).expect("Error renaming file");
        }
    }
}
